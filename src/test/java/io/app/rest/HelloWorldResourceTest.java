...
class HelloWorldResourceTest {
    ...
    static void setup() {
        ...
        RestAssured.requestSpecification = new RequestSpecBuilder().setContentType(ContentType.JSON)
                .setPort(ConfigProvider.getConfig().getValue("quarkus.http.test-port", Integer.class)).build();
    }
    ...
    public void test_ep_get_users() {
        given().when().get("/rest/hw/users").then().statusCode(200).body(is("{\"1\":{\"name\":\"foobar\"}}"));
    }
